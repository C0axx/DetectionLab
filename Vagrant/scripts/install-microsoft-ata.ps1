...
Invoke-Command -computername dc -Credential (new-object pscredential("PIRATESHIP\vagrant", (ConvertTo-SecureString -AsPlainText -Force -String "vagrant"))) -ScriptBlock {

    Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) [$env:computername] Installing the ATA Lightweight gateway on DC..."

    Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) [$env:computername] Adding wef.pirate.ship to hosts file..."
